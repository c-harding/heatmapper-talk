...
const elapsed = computed(() => {
  ...
  if (timerState.value.status === 'stopped' || !timerState.value.startedAt) return 0;
...
const frontmatter = (slide: SlideRoute) => (slide.meta?.slide as ({frontmatter?: Record<string, any>} | undefined))?.frontmatter;
...
const slideInfo = computed(() =>
  timedSlides.value.map(s => {
    const fm = frontmatter(s)!;
    const durationSeconds = (parseDuration(String(fm.sectionDuration)) ?? 0) * 60;
    return {
      no: s.no,
...
const durationOverride = useSessionStorage<number | null>('slidev-timing-duration-override', null);
const editingDuration = ref(false);
const durationInput = ref('');
const effectiveDuration = computed(() => durationOverride.value ?? totalDuration.value);
...
const sectionProgress = computed(() => {
  ...
  const sectionEnd = idx + 1 < timedSlides.value.length ? timedSlides.value[idx + 1].no : slides.value.length + 1;
...
const configEndTime = computed(() => ((configs as any)['end-time'] ?? (configs as any).endTime) as string | undefined);
const endTimeOverride = useSessionStorage<string | null>('slidev-timing-endtime-override', null);
...
const endTimeSeconds = computed(() => endTime.value ? parseClockTime(endTime.value) : null);

// Wall clock time when the timer started (seconds since midnight)
const startClockSeconds = computed(() => {
  if (timerState.value.status === 'stopped' || !timerState.value.startedAt) return null;
...
const endTimeSpan = computed(() => {
  if (startClockSeconds.value == null || endTimeSeconds.value == null) return null;
  let span = endTimeSeconds.value - startClockSeconds.value;
  if (span <= 0) span += 86400; // wrap past midnight
  return span;
});

const totalDurationString = computed(() => secondsToString(effectiveDuration.value));

const progressPct = computed(() => {
  if (timeDisplay.value === 'endTime' && endTimeSpan.value) {
    return Math.min(elapsed.value / endTimeSpan.value * 100, 100);
  }
  return effectiveDuration.value > 0 ? Math.min(elapsed.value / effectiveDuration.value * 100, 100) : 0;
});
...
function playToEnd() {
  ...
  nextTick(() => {
    const pauseIcon = document.querySelector<HTMLElement>('.slidev-presenter > .grid-container > .grid-section.bottom > :last-child .i-carbon\\:play');
    pauseIcon?.click();
  });
}

// --- Recorded slide durations ---
interface RecordedVisit { start: number; end: number }
interface RecordedSlideDuration { visits: RecordedVisit[] }
...
const recordedDurations = useSessionStorage<RecordedSlideDurations>('slidev-recorded-durations', {
  slides: {},
  currentSlide: -1,
  currentStart: -1,
});
...
function logRecordedDurations() {
  ...
  for (const section of slideInfo.value) {
    ...
    for (let no = sectionStart; no < sectionEnd; no++) {
      ...
      const title = frontmatter(slides.value.find(s => s.no === no)!)?.title ?? `Slide ${no}`;
      slideLines.push(`  ${no}. ${title}: ${parts.join(' + ')} = ${secondsToString(total)}`);
    }
    if (slideLines.length > 0) {
      lines.push(`${section.title} (${secondsToString(sectionTotal)} / ${secondsToString(section.durationSeconds)}):`);
...
watch(() => timerState.value.status, (newStatus, oldStatus) => {
  if (oldStatus === 'stopped' && (newStatus === 'running' || newStatus === 'paused')) {
    recordedDurations.value = { slides: {}, currentSlide: -1, currentStart: -1 };
...
<template>
  <Teleport v-if="isPresenter" defer to=".slidev-presenter > .grid-container > .grid-section.bottom > nav + div">
    <div class="flex flex-row items-stretch gap-2">
      <div class="flex flex-col flex-1 min-w-0">
        ...
        <div class="relative h-3 mx-2">
          <div
            ...
            :style="{ left: progressPct + '%', clipPath: 'polygon(50% 100%, 0 0, 100% 0)', backgroundColor: arrowColor }"
        ...
        <div class="flex text-xs rounded p-2 z-100 w-100%">
          <div
            ...
            :class="currentSection === slide.no ? 'bg-blue-300! pt-2 -mt-1.5' : ''"
          ...
          >
            ...
            />
            {{ slide.title }}<br/>{{ secondsToString(slide.durationSeconds * (timeDisplay === 'endTime' ? endTimeScale : durationScale)) }}
          </div>
        </div>
      </div>
      <fieldset class="flex flex-col text-xs gap-1 justify-center pl-2 w-16">
        <label class="flex items-center gap-1 cursor-pointer">
          ...
          <span v-if="!editingDuration" @dblclick="editingDuration = true; durationInput = String(effectiveDuration / 60) + 'm'">
            {{ totalDurationString }}
          </span>
          <input
            v-else
            ...
            @keydown.enter="durationOverride = (parseDuration(durationInput) ?? 0) * 60 || null; editingDuration = false"
            @keydown.escape="editingDuration = false"
            @blur="durationOverride = (parseDuration(durationInput) ?? 0) * 60 || null; editingDuration = false"
            autofocus
          />
        </label>
        <label class="flex items-center gap-1" :class="endTime ? 'cursor-pointer' : 'opacity-40'">
          <input type="radio" value="endTime" :disabled="!endTime" v-model="timeDisplay" />
          <span v-if="!editingEndTime" @dblclick="editingEndTime = true; endTimeInput = endTime ?? ''">
            {{ endTime ?? 'End time' }}
          </span>
          <input
            v-else
            ...
            class="w-8 bg-transparent border-b border-current outline-none"
            @keydown.enter="endTimeOverride = endTimeInput || null; editingEndTime = false; timeDisplay = 'endTime'"
            @keydown.escape="editingEndTime = false"
            @blur="endTimeOverride = endTimeInput || null; editingEndTime = false"
            autofocus
          />
        </label>
      </fieldset>
      <button
        ...
        @click="playToEnd"
      >
        <svg viewBox="0 0 16 16" class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="1">
